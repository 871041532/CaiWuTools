...
excel_2 = excelTool.read_excel("收入台账2020-5-1.xlsx")
sheet_1=excel_1.租金
...
def iter_func3(iter_data1):
    ...
    a1 =round_up( iter_data1.应收固租 or  0 )            #迭代表2行，应收租金列
    a2 =round_up( iter_data2["202006应收固租"] or 0 ) # 字典的202006应收固租列
    if a1 != a2:   #不等于
        s = "202006应收固租差异：%s %s 和我 %s "%(iter_data1.商户品牌, a1, a2)
        print(s)
    # print("报错：" + iter_data1.商户品牌 + str(a1) +"和" + str(a2) + "不同")

    b1= round_up(iter_data1.疫情减免 or 0  )             #迭代表2行，疫情减免列
    b2 = round_up(iter_data2["202006固租减免"]  or 0 )  # 字典的202006固租减免列
    if b1 != b2:  # 不等于
        s ="202006固租减免差异：%s %s 和我 %s "%(iter_data1.商户品牌, b1, b2)
        print(s)

    c1=round_up( iter_data1.应收物业 or 0)
    c2 = round(iter_data2["202006应收物业"] or 0)
    if c1 != c2:
        s ="202006应收物业差异：%s %s 和我 %s "%(iter_data1.商户品牌, c1, c2)
        print(s)

    d1= round_up(iter_data1.应收推广 or 0)
    d2 = round_up(iter_data2["202006应收推广"] or 0)
    if d1 != d2:
        s ="202006应收推广差异：%s %s 和我 %s "%(iter_data1.商户品牌, d1, d2)
        print(s)

    e1=round_up( iter_data1.已收固租 or 0)
    e2 =round_up( iter_data2["已收固租"] or 0)
    if e1 != e2:
        s ="已收固租 差异：%s %s 和我 %s "%(iter_data1.商户品牌, e1, e2)
        print(s)

    f1=round_up( iter_data1.已收物业 or 0)
    f2 =round_up( iter_data2["已收物业"] or 0)
    if f1 != f2:
        s ="已收物业 差异：%s %s 和我 %s "%(iter_data1.商户品牌, f1, f2)
        print(s)

    g1=round_up( iter_data1.已收推广 or 0)
    g2 =round_up( iter_data2["已收推广"] or 0)
    if g1 != g2:
