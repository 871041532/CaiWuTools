...
class ShowWindow(QWidget):
    ...
    def click_judge_btn(self):
        ...
        #    self.log("请先选择EAS编码基础信息表~(^v^)~", ShowWindow.LOG_ERROR)
            ...
            self.load_data()
            self.deal_zhifubao_data()
            # self.deal_weixin_data()
            self.log("处理完毕。")
    ...
    def load_data(self):
        ...
        duifanganwei_idx = data[0].index("对方账号")
        target_data = data[0:1]
        for i in range(len(data) - 1):
            idx = i + 1
            row = data[idx]
            if row[duifanganwei_idx] == "1001278619005510123	":
                target_data.append(row)
        self.my_data = target_data

    # target_data
    def deal_weixin_data(self):
            target_excel = {"凭证": []}
            ...
            row2 = ["公司", "记账日期", "会计期间", "凭证类型", "凭证号", "币种", "分录号", "对方分录号", "主表信息", "附表信息", "原币", "本位币", "报告币",
                    "主表金额系数",
            ...
            save_data(self.out_put_filename1, target_excel)
            # 处理日期
            Globals.eval_date_format(self.out_put_filename1, ["B", "C", "BN"])
